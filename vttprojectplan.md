...
- **Every step is a suggestion, and every step has a Skip (decided, revised after the first build).** Each step carries a **Skip** next to **Next**, and **Finish** is live from the first screen. A creation flow's job is to guide a build, not to police one, so nothing forces a player to spend everything, pick a set number of anything, or fill in a step they would rather come back to. Skip *clears* that step's contribution as well as moving on — otherwise it is Next with a different label — and Back is always there.
- **The presets, and what they are:** Teenager (8 Stat points, 3 Perks), Adult (16 / 5), Old Master (24 / 7). **The numbers are guidance, not gates.** Going over is a *warning* — shown in the wizard in amber, and posted to the Chat Log when the build lands, so the table finds out rather than having to notice — and then allowed. Even the preset itself is skippable: no preset means no budget at all, a free-form build with nothing to overspend.
- **What can still block is the short list of things that would leave a character broken**, and it is one item long today: a stance that is half-built or names the same Style twice, which is not a stance. Every one of those sits next to a Skip button, so nobody is ever trapped by one.
...
- **Learnability is not bypassed (bugfix found while building it).** `move:grant` refuses a styled Move to a character with no stance carrying that Style, and creation must not be a way around it — a flow that handed out any Move in the book would make the rule optional. The stance step runs *before* the Moves step precisely so a Move picked in the same sitting can be checked against the stance bought a moment earlier; the wizard flags an unlearnable Move before it is picked, and anything that still does not stick is named in the Chat Log rather than silently dropped.
...
- **Verified against the running app** (`scripts/playtest-character-creation.mjs`, 25 checks) plus a full browser pass: a complete Adult build applied end to end (Stats set, stance taken, Moves and Perks granted, answers saved, Stats locked, Max Stamina recomputed); an over-suggestion build allowed *and* warned about in chat; the emptiest possible build — every step skipped — still applying and still locking; an unknown Perk id dropped rather than counted; a same-Style stance refused; an off-Style Move refused and named; and a re-run re-stating the spread rather than doubling it.
