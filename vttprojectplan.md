...
- **Player (picked a character):** character list shows only characters with `character_type = 'pc'` to pick from at the door; once in, can view/edit any PC sheet (unrestricted, same as before — picking a character is about combat visibility, not a sheet-access lock). Can create new characters (always created as `pc`, no type selector shown).
