...
- **Everywhere OUTSIDE the Arena the four columns are behind a funnel button (decided, revised;
  ...
  - **The Arena keeps its open columns, deliberately.** Mid-round you are choosing a move against a
    clock, and a filter you have to open first is a filter you will not use. That is the whole
    difference between the two surfaces, and it is why this is not applied uniformly.
...
- **A move's frame cells are exactly as wide as the Tic squares under them (decided, revised again).**
  They were pinned to the Tic Counter's own 44px square while the strip below them grew to fill the
  board, so the two rulers drifted apart and a bar claiming Tics 1-3 no longer sat over the squares
  labelled 1, 2 and 3 — which is the entire content of that row. Matching takes three things agreeing,
  not one: the same leading label width, the same gap, and **nothing after the cells** — so the move's
  own name is taken out of flow at `md` and parked in the empty half-panel to the right of the board,
  because in flow it was eating 176px the strip below never had to give up. The height stays short, so
  more moves fit. Below `md` this all reverts to fixed cells: a phone's board is full width with
  nowhere beside it to park a name, and seven `flex-1` cells in 200px would be 25px each.
