...
- **Tags (decided)**: each move carries 0-10 Tags, picked from the world-level GM-managed `tags` list (created/edited in the Compendium, like Tells — this pulls the base tag tables forward from Phase 4; per-character tag overrides via Perks remain Phase 4). Tags can also change dynamically later (Perks adding Tags to specific moves). A Tag has a **name and an optional description**; hovering a Tag anywhere it's shown (the Tag manager, a Move Creator's picker, a Move card) pops a tooltip with that description. **On a Move card specifically, a Tag with a description is also tappable (decided, fix) — `TapDescribe.jsx`.** A `title` attribute has nothing that opens it on a touch device, so a Tag's description was unreachable there at all; reported as "I cannot tap to read a Move tag." Wraps a chip so a tap opens the same text in a small portalled popover, anchored with the same `useAnchoredPosition` hook every other floating panel here uses, closed by tapping it again, tapping elsewhere, or Escape — the plain `title` stays too, so a mouse still gets a hover tooltip for free. A chip with no description renders as an inert `<span>`, unchanged, since there is nothing to tap open. `MoveCard`'s Combat Style badge got the same treatment for the same reason (its own hover-only explanation of what the badge means). One `pointer-events-none` exception, unaffected by this: the Arena's own "read this move" info popovers (opened by the ⓘ on a declare-picker chip, and a declared move's own hover-to-expand card) are deliberately non-interactive so they never steal a drag or a tap-to-place gesture — a Tag inside either of those stays exactly as un-tappable as it always was, by the same design that makes the card itself unclickable.
...
- **A modifier modifies the ROLL, not each die (decided, fix; implemented).** Every modifier in the game — the ad-hoc one typed into a roll dialog, Reasons to Fight, the Stance matchup, a move's own Roll Modifier, a Perk's per-move bonus — was added to **each die separately**, so a move rolling Skull + Body + Brain at +4 collected +12. It is summed once and added to the total now (`rollTotal` in `server/gameLogic.js`, unit-tested).
  ...
  - **...but it WAS a special case in the chat log, and that was a bug (fix).** The swing rode the
    roll's `total` while the chat card was handed `mod` alone, so a follow-up whose grab had been read
    printed `11 + 4 = 15` and the very next system line announced 10 — the log flatly contradicting
    itself, reported from the table exactly that way. `logRoll` is now handed `mod + chainRollBonus`
    at both call sites (`rollFor` and `resolveAttack`); the roll **event** still carries `mod` and the
    swing separately, because the cutscene lists the read as its own named term.
  - **The chat card itemises its modifier too, now (decided, new).** The named pieces already rode the
    `roll` event for the cutscene's benefit; the chat log got a bare sum, so a modifier made of a
    Stance matchup *and* a read on the grab was a number nobody could account for. `logRoll` carries
    `modifierTerms` on the same `payload` column the roll-context uses (merged, not replacing it), and
    the card prints them on a quiet second line under a total that stays big — `formatModifierTerms`,
    a sibling of the cutscene's `formatRollBreakdown` that drops the sum and the total, because one
    long string at `text-2xl` in a 320px panel is not a total any more. Empty, and so invisible, when
    the modifier is one plain thing — which is most rolls.
...
- **Cancelling a declared move (decided):** while still in the Declaration Phase and before that move's Stamina Cost has been committed (`stamina_committed = 0` — i.e. before the declaring character themselves presses **Done Declaring**), the declaring player can retract a just-declared move and declare something else instead. A small ✕ button on the move's flip card (rendered outside the flip animation itself, so it stays visible no matter which face happens to be showing — the declaring player's own view already renders the fully-revealed face immediately, well before commitment, so a cancel button placed only on the Tell face would be invisible to exactly the person who'd want it) calls `move:undeclare`, deleting the `declared_moves` row outright. Once Stamina Cost is committed (`stamina_committed = 1`), the move is locked in and can no longer be cancelled — the ✕ stops rendering. See `move:undeclare` under Real-time events below.
  - **The ✕ itself was a 16px circle (decided, fix).** `CompactDeclaredMoveCard`'s cancel button never picked up the 44px mobile touch-target bump the rest of the app's icon buttons got in Mobile Readiness §14.10 — reported as "hard to remove a Declared move." It is now `h-7 w-7` on mobile (`md:h-4 md:w-4` keeps the original compact size on desktop), the same shape as every other icon button's mobile bump: bigger tap target, unchanged desktop density.
...
- **The phone version of this row is now the same funnel-button popover every other filtered list uses
  (decided, reversed).** It used to keep an always-open compact in-panel row (`md:hidden` on the row,
  `hidden md:flex` on the columns), on the reasoning that a filter you have to open first is a filter
  you will not use mid-round — the same reasoning that still holds the desktop columns open (see below).
  Reported from the table as costing too much of the phone screen on its own: four rows of chips before
  a single move card, stacked on top of the Tell/Default-Unique tabs above it and the round clock
  already running. The desktop columns are untouched — this is mobile only, and it is a real reversal of
  the reasoning above for that one width, made on the table's explicit say-so rather than re-derived from
  it. The row is now a `MoveFilterPopover` (`md:hidden`, same component the Compendium and a character's
  Moves tab already use), wrapping the same four `MoveFilterChips` rows in the DOES/IS order those
  surfaces use, reading from the exact same `list.filters` state the desktop columns read from — so
  filtering behaves identically, only the chrome around it changed.
...
- **One column per filter — four columns, two a side (revised).** The first cut stacked two filters
  ...
  room — the labels they carry are short). The Arena's phone layout keeps its two compact rows a
  side inside the panel, which has no side space to give either way.
- **Everywhere OUTSIDE the Arena the four columns are behind a funnel button (decided, revised;
  `MoveFilterPopover.jsx`).** The `sm:grid-cols-2 lg:grid-cols-4` grid the Compendium and the sheet's
  Moves tab used ate the top of the page before a single move was visible — the Tag column alone runs
  to twenty-odd chips — and both are pages you *browse*. They now get one button with a funnel on it
  and a panel that opens from it; the rows inside are full width and stacked in the same order the
  columns were in (what the move DOES, then what it IS).
  - **The Arena's DESKTOP columns stay open, deliberately.** Mid-round you are choosing a move against a
    clock, and a filter you have to open first is a filter you will not use — that is why this popover
    treatment is not applied uniformly at every width. It **is** applied to the Arena's own mobile row
    now, though (see above): a phone has no side space for a column either way, and the table's own
    verdict on a phone specifically was that the always-open row cost more screen than the beat spent
    opening a popover.
  - **The button is sticky** (`sticky top-0` inside `<main>`'s own scroller), so it is still reachable
    two hundred moves down rather than scrolling away with the header it belongs to. It carries a
    **count of active filters** and a `clear all` beside it: a collapsed filter showing eleven of
    ninety moves must say why on its face, or the missing moves read as a bug.
  - **The panel is portalled to `<body>`** and placed by the shared `useAnchoredPosition` — both call
    sites sit inside `<main class="overflow-y-auto">`, which clips on both axes, and the sticky
    wrapper is itself a positioning context the panel would be trapped in. Same trap, same fix, as
    every other floating panel here.
  - It takes arbitrary `children` rather than owning the filters, which is what lets the Compendium's
    icon-only **Style** row (the one control here that is not `MoveFilterChips`) sit inside it with no
    special case — and what let the **Perks Compendium's** Tag row move behind the same button, so the
    two libraries under one tab strip do not answer the same question two different ways.
  - **The Character Creation dialog is left alone**: its filters are already in a side rail beside a
    bounded list, so they cost no vertical space and the complaint does not apply.
...
**Unselected chip contrast, raised, everywhere a move is filtered (decided, fix).** Reported from the
table as "the frames and letters in the filters… are too dark and hard to read." A picked chip already
read fine (`brand-300` text on a lit `brand-600/30` fill), but the far more common **unpicked** state —
every chip nobody has tapped yet — was `border-zinc-700/800 text-zinc-500` (`MoveFilterChips`) or
`border-zinc-800 text-zinc-400` (`MoveFilterColumn`) against this app's near-black panels, neither of
which clears WCAG's contrast floor on a dark theme. Both, plus the Compendium's own bespoke Style icon
row (the one filter that isn't built from either shared component), now read `border-zinc-500
text-zinc-300` unpicked, with a lighter hover state to match — one shared fix, since all three surfaces
answer the same complaint. The section labels ("Attack Target:", a column's own heading) and the "clear"
links picked up the same one-step lift. This applies to every one of these filters at both PC and mobile
widths — nothing here is breakpoint-gated.
...
- **Every Tic in the counter is a button.** Clicking one **pins** the round to the state immediately
  *before* that Tic — stats, Stamina and the move bars together, not just the log position — stops the
  ...
  from.
- **The two side columns split at the panel's exact centre (decided, fix).** They were `1fr 1fr`
  after the Tic column, which put the divider a couple of centimetres right of where the two fighters'
  Stat cards meet directly above it — two rules for one "left side / right side" line, and the eye
  reads the mismatch long before it works out why. The grid is `[3.5rem calc(50% - 4.25rem) 1fr]`
  instead: 50% less the Tic column and half its gap, so the boundary lands dead centre at any width.
- **The Stat cards fold away** behind a small chevron button beside *Skip to end*, leaving the Tic
  counter and the move frames. The frozen header carries the counter, the frames AND both fighters'
  full eight-pip grids, and on a laptop that is most of the screen before one line of the log is
  visible. Per-viewer and remembered (`localStorage`, `vtt-cutscene-stats-collapsed`, beside the
  playback speed in `theme.js`) for the same reason the speed is: how much header you want is a
  property of the person watching and the screen they are on. Both accessors are wrapped in
  `try/catch` — a private window makes `localStorage` itself throw, and a replay that will not render
  because a preference could not be read is a worse bug than a preference that does not stick.
- **A move's frame cells are exactly as wide as the Tic squares under them, at every size (decided,
  revised a third time — fix).** They were pinned to the Tic Counter's own 44px square while the strip
  below them grew to fill the board, so the two rulers drifted apart and a bar claiming Tics 1-3 no
  longer sat over the squares labelled 1, 2 and 3 — which is the entire content of that row. Matching
  takes three things agreeing, not one: the same leading label width, the same gap, and the same fixed
  budget of whatever sits after the cells — so at `md` the move's own name is taken out of flow entirely
  and parked in the empty half-panel to the right of the board, because in flow it was eating 176px the
  strip below never had to give up. The height stays short, so more moves fit.
  **Below `md` this used to revert to a fixed `w-8` per cell, and that was a bug (reported: "move tics in
  the mobile cutscene are not aligned to the Tic counter" and "I can drag to move the cutscene
  horizontally, while there is nothing there").** A phone has no side panel to park the name in, so it
  stayed inline — but fixed-width cells could not shrink to make room for it, so the row was simply wider
  than the phone (the width nobody meant to add was the overflow you could drag into). Both rows now
  compute their cells as `flex-1` off an **identical** fixed budget of non-cell siblings: the same leading
  label width on both, and a **short trailing name box (`w-16`, down from `w-28`) on the move row,
  mirrored by an invisible spacer of the same width on the Tic row** that exists purely so the two rows
  divide the same remaining pixels the same way. Squishing the name down is the trade the table asked
  for directly — it truncates harder, and the row fits the screen without a horizontal drag.
...
- **App shell (`client/src/App.jsx`):** below `md:`, the desktop header nav links/search bar/Chat-toggle button (`hidden md:inline-block`/`hidden md:block`) are replaced by a hamburger "More" menu (Search full-width + Rules + Settings links) and a 4-item **bottom nav** (**14.2A**: bottom nav, not a hamburger-only menu — Arena/Characters(or Character)/Compendium/Chat, the same four destinations the desktop header already always shows). **The bottom nav stays mounted at all times, including while Chat is open (decided, revised).** It used to unmount the moment Chat opened, which left the panel's own ✕ — in the opposite corner from the Chat button that opened it — as the only way out, and no way to navigate anywhere else without closing Chat first. This required the chat overlay to be positioned `absolute` against the content row (which is now `relative`) rather than `fixed` against the viewport, so it sits below the header and *above* the nav instead of covering it. **Chat is a real tab on mobile, and the mobile ✕ is gone (decided, revised again).** Half-measures made it read as neither panel nor page: navigating to another tab left Chat sitting open on top of it, and the ✕ was a second, differently-placed exit for something the Chat tab already toggles. Now any change of route closes Chat below `md:` — keyed off the pathname, so the back button and any in-app link behave identically — and tapping Chat again is the only way out, exactly like every other tab.
  - **A tab you were already on didn't count as "a change of route" (decided, fix).** The close effect is keyed off `location.pathname` actually changing, but opening Chat never changes the route underneath it — so pressing the SAME tab Chat was opened from (the obvious "take me back" gesture, and a common case: Chat is usually opened while already on the Arena or Compendium) left the pathname unchanged, the effect never fired, and Chat stayed open over a page that looked like it should have taken over. Reported as "make the Chat tab a proper tab… pressing on other tabs closes it." Every nav `Link` in the bottom strip (and the mobile "More" menu's Rules/Settings links) now also closes Chat directly on every press via its own `onClick`, regardless of whether the destination is the current route — the pathname effect still covers the back button and any other in-app navigation that doesn't go through these links.
  - **Reopening Chat could pop a round replay back open uninvited (decided, fix).** `ChatPanel` never actually unmounts — `App.jsx` always renders it, and `open={false}` just makes it return `null` — so its own local state (which round replay dialog, if any, is open) survived being hidden. Closing Chat on mobile (by any of the routes above) now also resets that state, so tapping back into Chat never resurrects a `DialogShell` the player thought they had closed. Desktop is untouched, matching the point below: Chat never "closes" there, so a replay left open should stay open.
...
- **"Hide locked moves" (decided, new):** a toggle button at the top of the Moves step hides every bundle that is currently unlearnable — the identical `blocked` rule (every variant in the bundle fails the Style check) that already dims the row and shows its `needs a Style` badge, just filtered into its own list rather than skipped inline, so the empty-state message reflects the filter too and the button can never disagree with what a visible row shows. Off by default; toggling it back shows every bundle again, locked or not.
...
- **Mechanical effects are code, one file per Perk, bound by the Perk's exact NAME (decided, revised — the third arrangement and the one to keep).** Read `server/perks/index.js` first; it carries the doctrine beside the registry, which is where it stays current.
  ...
  - **A seventh batch — seven Perks for the second official playtest (decided, new).** Five of them
    needed nothing but existing seams, which is the register doing its job; two needed the engine to
    learn something it did not know.
    - **Rhythm Breaker** — the mirror of Dogfighter on the other half of the same contest: all your
      Moves count as **Interrupter (2)**. Its two sentences come to the same arithmetic for the same
      reason Dogfighter's do, so it is a flat +2 with no rule for their meeting. One file, one line.
    - **Anime Protagonist** — **+1 more per Reason to Fight**, on top of the engine's own +1 per
      Reason. Written as a second additive term rather than as a multiplier on the first, because a
      seam that multiplies is a seam with an ordering question hidden inside it. `reasonsToFight` is
      now threaded into the roll context so the Perk can read what the engine already looked up.
    - **Never Tell Me the Odds** — **+5 while you are outnumbered**, from a new `sideCounts` fact on
      the roll context (how many fighters on each side of your own pair).
    - **No Wasted Movements** — **one less imposed Recovery frame, from every source**, on the new
      `imposedRecoveryDelta` seam. Asked at the two doors rather than at the effects, so a source
      added later is covered by arriving through the door.
    - **Tip Top Shape** — Stamina that would go **over your cap** banks until it is worth a Step of
      healing (5 per Step), on the new `staminaOverflowHealing` seam. **The remainder banks**
      (decided): overflowing by 3 heals nothing yet and keeps the 3, because "for every 5" reads as a
      running total, and without it the Perk is worth almost nothing to anybody whose Stamina die is
      small. Per grant, scoped to the fight.
    - **Yamazaki Black Bones — the Minimum Damage Threshold, per Stat (new engine work).** A Stat at
      **d6 or lower** is +3 harder to hurt: the bar for damage landing on *that* Stat goes 5 → 8,
      while its neighbours on the same fighter are untouched. That is what `minDamageThresholdWhenAttacked`
      could not express — it answers once for the whole exchange, which is right for a hide and wrong
      for a bone — so `statDamageThreshold` is asked once per Stat a blow is about to touch and summed
      on top of the exchange-wide figure. A fighter carrying both Perks simply gets both.
      - **Read off the die SIZE, not the printed Stat.** A d6+2 is a d6 somebody is good with; the
        bone is still small, and the bonus is a separate additive thing everywhere else in the game.
      - **The Threshold is a gate and only a gate** (`computeHitDamage`), so a blow that clears 8 hurts
        a d6 for exactly what it was always worth. The Perk never softens a hit; it stops the ones that
        were barely there.
      - **A stopped Stat gets its own `damage_shrugged` event**, not `damage_unapplied`. Nothing was
        owed to that Stat, and the end-of-round Injury report reads `damage_unapplied` back off the
        log — folding the two would have it counting bones that were never hurt. The gate is also
        checked **after** the broken-Stat line, since a destroyed die sits at the floor and would
        otherwise be swallowed by every "small bone" test there will ever be.
      - Resolved inside `applyAutoDamage`, which is the only place that knows which concrete Stats a
        blow is about to touch and reads each die *after* every earlier hit this round stepped it down.
        Two extra queries, both skipped entirely when nobody in the fight has such a Perk — this
        function is on the path of every blow.
    - **Never a Fool — a Feint aimed at you announces itself (new disclosure work).** While it is still
      an unrevealed Tell on the strip, you know that particular move is a Feint. **And nothing else**:
      not which move, not its frames, not its Roll.
      - It rides the declared-move payload as `isFeint`, **absent rather than false** on every row the
        Perk does not earn — the same protect-by-absence rule `moveId`, `moveName` and Eye Catcher's
        `attackHeights` already follow, because a flag a devtools reader can flip is not a secret. The
        same three gates Eye Catcher uses decide what "at you" means.
      - **The move the Feint MASKS stays off the wire entirely.** Knowing you are being lied to is not
        knowing the truth, and that is exactly what keeps the Feint Tag worth having against somebody
        carrying this.
      - On the Tic Counter it **recolours the existing telegraph glow red** rather than drawing a
        second marker — what changes is that you know the commitment is a lie, not that there is one —
        with a ghostly **"Feint!"** over it on hover and the Tell connector drawn in the same red.
        `MoveLinkOverlay` reads that off a `data-feint` attribute on the anchor element it already
        looks up, so there is no second registry to keep in step with the first.
      - Which declared moves carry the Tag is resolved for the whole board in **two queries**, not the
        two-per-move `moveTagNamesFor` does: this is asked on a broadcast, where wall time is depth
        times round-trip. Per character as well as per move, since a Perk can add or remove a Tag for
        one fighter.
      - Verified by `scripts/playtest-never-a-fool.mjs` rather than a unit test, for the reason Eye
        Catcher's probe gives: what the Perk changes is who is entitled to which key, and the only way
        to check an entitlement is to ask as somebody who does not have it.
  - **Endless Possibilities, automated on its own (decided, new).** Learn and use any Move regardless of its own Style requirement — your own stance still applies to everything else a Style governs. A new boolean-OR seam, **`bypassesStyleRequirement`**, checked alongside the existing `characterHasStyle` test at all three of that check's enforcement sites: `character:create`'s move-grant loop, the `move:grant` handler (learning), and `move:declare` (using it, from a stance that still doesn't carry the Style). One file, unconditional (owning it is the whole condition, same shape as Genius Observer), same as every other seam Perk here.
...
7. **Settings** (decided, new) — reachable via the header's Cog icon, visible to every role (no role-gating on the page or its settings so far). Two settings:
   ...
   - **Scene Stage** (decided, new — `client/src/lib/sceneSettings.js`): four sliders/toggle controlling how the Scene tab's stage (`StageRoster.jsx`) renders on *your own* screen — everyone still sees the same Scene and the same summons, just sized to their own taste. **Character Height** (0.5×–1.5×, default 1×) scales `StageRoster`'s own base 70dvh figure height. **Distance Apart** (0×–3×, default 1×) scales `sceneLayout.js`'s `SLOT_GAP`. **Picture Size** (0.5×–2×, default 1×) scales `sceneLayout.js`'s `SLOT_WIDTH` — its nominal per-character horizontal spacing/crowding unit, not a crop or a second sizing rule competing with Character Height. **Show name plates over characters** (checkbox, on by default) toggles a `HaloText` label (the same "readable over a busy background" component the Relationships board already uses) above each summoned figure's own head. All four are `localStorage`-only, read once per `/scene` mount (a fresh visit picks up any change made on the Settings page since) and handed down through `ScenePage.jsx` as plain props — `sceneLayout.js`'s pure layout math takes `slotWidth`/`slotGap` as plain overridable parameters and has no idea a setting exists.

   All settings persist per browser. More are expected to land here; nothing about the page structure is specific to any of them.
...
**Ninth post-overhaul batch — done.** One mobile bugfix:
- **The Counters popup no longer hides under the mobile footer nav (decided, fix).** `CounterAdjustDialog` opened as `DialogShell`'s ordinary bottom sheet, which anchors to the screen's own bottom edge — a position `BottomNav`'s fixed footer was never accounted for, so its lower portion rendered underneath the nav bar on a phone-width viewport. `DialogShell` gained a `centered` prop (forces `items-center` instead of the sheet's usual `items-end md:items-center`, and full `rounded-2xl` corners instead of the bottom sheet's flat-bottomed `rounded-t-2xl`, since a flat bottom only reads correctly anchored to the real bottom edge) plus this dialog now also opts into the pre-existing `portal` prop, since it opens from inside `ChatPanel`'s own mobile-only stacking context. Verified live: the dialog now sits with equal space above and below on a 390×844 viewport, clear of the footer nav.
...
- Real character/move/perk/Tell art (commissioned images to replace placeholders) — the palette/font/corner/animation conventions themselves are now decided, see Global UI — Visual Theme above. The user-facing color-customization setting once flagged here as future work is now built, and two more have since landed alongside it — Cutscene Speed and Scene Stage (see Pages / views — Settings above); more settings are still expected to land here over time, with nothing specific beyond those planned yet
