...
**Implementation note:** the placement/reveal/overflow math above is built and unit-tested in isolation, as a bare pure-function module — `server/combatTiming.js` (`resolveSideInitiative`, `computePlacementTic`, `computeMoveFootprint`, `isMoveRevealedTo`, `relativeTic`) — before any socket/DB/Arena wiring, per the Implementation Risks section's recommended approach. `resolveSideInitiative`'s tied-initiative case isn't specified above; it's broken by having the `left` side declare first (`right` wins ties) — an arbitrary but deterministic default, easy to revisit once this is actually wired into play.
