  ...
  return (
    <div className="flex h-screen flex-col items-center justify-center gap-8 bg-zinc-950 text-zinc-100">
      ...
      <div className="flex w-full max-w-3xl items-stretch gap-6 px-6">
        <div className="flex min-w-0 flex-1 flex-col gap-2 rounded-xl border border-zinc-800 bg-zinc-900 p-4">
          <span className="text-xs font-bold uppercase tracking-wide text-zinc-500">Play as…</span>
              ...
              pcs.map((c) => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center gap-3 rounded-lg border border-zinc-800 bg-zinc-950 p-2 text-left hover:border-sky-600 hover:bg-zinc-800 active:scale-[0.98] transition"
                  >
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-md bg-zinc-800 text-sm font-bold text-zinc-500">
                      {src ? (
                    ...
                    <span className="min-w-0 truncate font-semibold text-zinc-200">{c.name}</span>
                  </button>
                );
              })
            )}
        ...
        <button
          onClick={chooseGm}
          className="shrink-0 rounded-xl bg-amber-700 px-10 py-6 text-2xl font-bold hover:bg-amber-600 active:scale-95 transition"
        >
          GM
        </button>
      </div>
